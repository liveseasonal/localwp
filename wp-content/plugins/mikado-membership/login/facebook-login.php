...
if ( ! function_exists( 'search_and_go_elated_get_facebook_social_login' ) ) {
	...
	function mkdf_membership_get_facebook_social_login() {
		...
		$facebook_login_enabled = voyage_mikado_options()->getOptionValue( 'enable_facebook_social_login' ) == 'yes' ? true : false;
		...
		if ( ! is_user_logged_in() && $enabled ) {
			...
			$html = '<form class="mkdf-facebook-login-holder">'
			        ...
			        voyage_mikado_get_button_html( array(
				        'html_type'    => 'button',
...
if ( ! function_exists( 'mkdf_membership_check_facebook_user' ) ) {
	...
	function mkdf_membership_check_facebook_user() {
		...
		if ( isset( $_POST['response'] ) ) {
			...
			mkdf_membership_ajax_response( 'success', esc_html__( 'Login successful, redirecting...', 'mikado-membership' ), $url );
		}
		wp_die();
