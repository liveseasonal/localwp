	...
	function mkdf_membership_set_social_vars() {
		...
		if ( $social_login_enabled ) {
			...
			$social_variables = array(
				'facebookAppId'  => voyage_mikado_options()->getOptionValue( 'enable_facebook_social_login' ) == 'yes' ? voyage_mikado_options()->getOptionValue( 'enable_facebook_login_fbapp_id' ) : null,
				'googleClientId' => voyage_mikado_options()->getOptionValue( 'enable_google_social_login' ) == 'yes' ? voyage_mikado_options()->getOptionValue( 'enable_google_login_client_id' ) : null
			);
...
if ( ! function_exists( 'mkdf_membership_login_user_from_social_network' ) ) {
	...
	function mkdf_membership_login_user_from_social_network( $email, $nonce, $network ) {
		...
		if ( ! is_wp_error( $user ) ) {
			...
			esc_html_e('Not valid user', 'mikado-membership');
		}
