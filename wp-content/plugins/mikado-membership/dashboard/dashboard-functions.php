	...
	function mkdf_membership_get_dashboard_navigation_items() {
		...
		$items         = array(
			'profile'      => array(
				...
				'text' => esc_html__( 'Profile', 'mikado-membership' )
			),
			'edit-profile' => array(
				'url'  => esc_url( add_query_arg( array( 'user-action' => 'edit-profile' ), $dashboard_url ) ),
				'text' => esc_html__( 'Edit Profile', 'mikado-membership' )
			)
		);
				...
				if ( $user_id ) {
					...
					if ( ! empty( $update_data['password'] ) ) {
						if ( $update_data['password'] === $update_data['password2'] ) {
							...
							mkdf_membership_ajax_response( 'error', esc_html__( 'Passwords don\'t match', 'mikado-membership' ) );
						}
					}
					...
					if ( ! empty( $update_data['email'] ) && filter_var( $update_data['email'], FILTER_VALIDATE_EMAIL ) ) {
						...
						mkdf_membership_ajax_response( 'error', esc_html__( 'Error. Please insert valid email', 'mikado-membership' ) );
					}
					...
					mkdf_membership_ajax_response( 'success', esc_html__( 'Your profile is updated', 'mikado-membership' ), $dashboard_url );

				} else {
					mkdf_membership_ajax_response( 'error', esc_html__( 'You are unauthorized to perform this action.', 'mikado-membership' ) );
				}

			} else {
				mkdf_membership_ajax_response( 'error', esc_html__( 'Error.', 'mikado-membership' ) );
			}
