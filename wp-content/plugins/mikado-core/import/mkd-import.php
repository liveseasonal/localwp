...
class VoyageMikadoImport {

    public $message = "";
    public $attachments = false;
    function VoyageMikadoImport() {
    ...
    public function import_content($file){
        if (!class_exists('WP_Importer')) {
            ...
            $this->message = esc_html__("Error loading files", 'mikado-core');
        }
    }
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'mkd_options_voyage', $options);
    ...
    public function file_options($file){
        ...
        /*if ( file_exists($file_for_import) ) {
            ...
            $this->message = esc_html__("File doesn't exist", 'mikado-core');
        }*/
    ...
    function mkd_admin_import() {
        if (mkd_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'voyage_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array($voyage_mikado_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'voyage_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'voyage_mikado_enqueue_admin_styles');
...
function mkd_init_import_object(){
    global $voyage_mikado_import_object;
    $voyage_mikado_import_object = new VoyageMikadoImport();
}

add_action('init', 'mkd_init_import_object');

if(!function_exists('voyage_mikado_dataImport')){
    function voyage_mikado_dataImport(){
        global $voyage_mikado_import_object;

        if ($_POST['import_attachments'] == 1)
            $voyage_mikado_import_object->attachments = true;
        else
            $voyage_mikado_import_object->attachments = false;

        $folder = "voyage/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $voyage_mikado_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_mkd_dataImport', 'voyage_mikado_dataImport');
}

if(!function_exists('voyage_mikado_widgetsImport')){
    function voyage_mikado_widgetsImport(){
        global $voyage_mikado_import_object;
        ...
        $voyage_mikado_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_mkd_widgetsImport', 'voyage_mikado_widgetsImport');
}

if(!function_exists('voyage_mikado_optionsImport')){
    function voyage_mikado_optionsImport(){
        global $voyage_mikado_import_object;
        ...
        $voyage_mikado_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_mkd_optionsImport', 'voyage_mikado_optionsImport');
}

if(!function_exists('voyage_mikado_otherImport')){
    function voyage_mikado_otherImport(){
        global $voyage_mikado_import_object;
        ...
        $voyage_mikado_import_object->import_options($folder.'options.txt');
        $voyage_mikado_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $voyage_mikado_import_object->import_menus($folder.'menus.txt');
        $voyage_mikado_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_mkd_otherImport', 'voyage_mikado_otherImport');
}