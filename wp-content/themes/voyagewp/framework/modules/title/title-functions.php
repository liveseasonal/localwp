...
if(!function_exists('voyage_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function voyage_mikado_get_title() {
		$id = voyage_mikado_get_page_id();

		extract(voyage_mikado_title_area_height());
		extract(voyage_mikado_title_area_background());
		...
		if(($meta_temp = get_post_meta($id, "mkdf_show_title_area_meta", true)) !== "") {
			...
			$show_title_area = voyage_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_hide_title_text_meta", true)) !== "") {
			...
			$hide_title_text = voyage_mikado_options()->getOptionValue('hide_title_text') == 'yes' ? true : false;
		}
		...
		$parameters = apply_filters('voyage_mikado_title_area_height_params', $parameters);

		voyage_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if(!function_exists('voyage_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines voyage_mikado_title_text filter
	 ...
	 */
	function voyage_mikado_get_title_text() {
		...
		if(is_tag()) {
			...
			$title = single_term_title("", false).esc_html__(' Tag', 'voyage');
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'voyage')." ".get_the_author();
		...
		elseif(is_search()) {
			...
			$title = esc_html__('Search', 'voyage');
		} //is current page 404?
		elseif(is_404()) {
			//is 404 title text set in theme options?
			if(voyage_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = voyage_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'voyage');
			}
		} //is WooCommerce installed and is shop or single product page?
		elseif(voyage_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif(voyage_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'voyage');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('voyage_mikado_title_text', $title);

		return $title;
	}
}

if(!function_exists('voyage_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see voyage_mikado_get_title_text()
	 */
	function voyage_mikado_title_text() {
		echo voyage_mikado_get_title_text();
	}
}

if(!function_exists('voyage_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function voyage_mikado_subtitle_text() {
		$id       = voyage_mikado_get_page_id();
		$subtitle = '';
...
if(!function_exists('voyage_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see voyage_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function voyage_mikado_custom_breadcrumbs() {
		global $post, $wp_query;

		$output      = "";
		$homeLink    = esc_url(home_url('/'));
		$pageid      = voyage_mikado_get_page_id();
		$bread_style = "";
		...
		$delimiter   = '<span class="mkdf-delimiter" '.voyage_mikado_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
		$home        = get_bloginfo('name'); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before      = '<span class="mkdf-current" '.voyage_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
		$after       = '</span>'; // tag after the current crumb

		if(is_home() && !is_front_page()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.voyage_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter.' <a '.voyage_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.get_the_title($pageid).'</a></div></div>';

		} elseif(is_home()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner">'.$before.$home.$after.'</div></div>';
		} elseif(is_front_page()) {
			if($showOnHome == 1) {
				$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.voyage_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a></div></div>';
			}
		} else {

			$output .= '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.voyage_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter;

			if(is_category() || voyage_mikado_is_product_category()) {
				...
				$output .= $before.esc_html__('Search results for ', 'voyage').'"'.get_search_query().'"'.$after;

			} elseif(is_day()) {
				$output .= '<a '.voyage_mikado_get_inline_style($bread_style).' href="'.get_year_link(get_the_time('Y')).'">'.get_the_time('Y').'</a>'.$delimiter;
				$output .= '<a '.voyage_mikado_get_inline_style($bread_style).' href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F').'</a>'.$delimiter;
				...
				$output .= '<a '.voyage_mikado_get_inline_style($bread_style).' href="'.get_permalink($parent).'">'.$parent->post_title.'</a>';
				if($showCurrent == 1) {
				...
				while($parent_id) {
					...
					$breadcrumbs[] = '<a '.voyage_mikado_get_inline_style($bread_style).' href="'.get_permalink($page->ID).'">'.get_the_title($page->ID).'</a>';
				...
				$output .= $before.esc_html__('Posts tagged ', 'voyage').'"'.single_tag_title('', false).'"'.$after;

			} elseif(is_author()) {
				global $authordata;
				$output .= $before.esc_html__('Articles posted by ', 'voyage').$authordata->display_name.$after;
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'voyage').' '.get_query_var('paged').")".$after;
...
if(!function_exists('voyage_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function voyage_mikado_get_title_area_height() {
		$id         = voyage_mikado_get_page_id();
		$title_size = voyage_mikado_get_meta_field_intersect('title_text_size', $id);
		...
		} elseif(voyage_mikado_options()->getOptionValue('title_area_height') !== '') {
			$title_height = voyage_mikado_options()->getOptionValue('title_area_height');
		}

		return apply_filters('voyage_mikado_title_area_height', $title_height);
	}
}

if(!function_exists('voyage_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function voyage_mikado_get_title_content_padding() {
		return apply_filters('voyage_mikado_title_content_padding', 0);
	}
}


if(!function_exists('voyage_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function voyage_mikado_title_area_height() {
		$id                            = voyage_mikado_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = voyage_mikado_get_title_area_height();
		$header_height_padding         = voyage_mikado_get_title_content_padding();
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_vertial_alignment_meta", true)) !== '') {
			...
			$title_vertical_alignment = voyage_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
...
if(!function_exists('voyage_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function voyage_mikado_title_area_background() {
		$id                           = voyage_mikado_get_page_id();
		$show_title_img               = true;
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_color_meta", true)) != "") {
			...
			$background_color = voyage_mikado_options()->getOptionValue('title_area_background_color');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_image_meta", true)) != "") {
			...
			$background_image = voyage_mikado_options()->getOptionValue('title_area_background_image');
		}

		//check for background image width
		$background_image_width = "";
		if($background_image !== '') {
			$background_image_width_dimensions_array = voyage_mikado_get_image_dimensions($background_image);
